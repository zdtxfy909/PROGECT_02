...
<script>
	...
	function write_go() {
		location.href = "controller?type=write";
	}
	function logout_go() {
		...
		<tbody>
			...
			<tr>
				<c:forEach var="vo" items="${list }">
					<td>${vo.postIdx }</td>
					<td>${vo.postType }</td>
					<td><a href="detail.jsp?postIdx=${vo.postIdx}">${vo.title }</a></td>
					<td><a href="detail.jsp?postIdx=${vo.postIdx}">${vo.postContent }</a></td>
					<td>${vo.postPhoto }</td>
					<td>${vo.name }</td>
					<td>${vo.hit }</td>
					<td>${vo.likeNum }</td>
